...
! !EOP
   ...
   real(R8)    :: vmag       ! surface wind magnitude   (m s-1)
   real(R8)    :: vmag_old   ! surface wind magnitude with lower limit and
                             ! convective correction    (m s-1)
   ...
   DO n=1,nMax
     ...
     !-----Calculate some required near surface variables.---------
        ! EQN (18)                           
        vmag_old = max( umin, sqrt( (ubot(n)-us(n))**2 + (vbot(n)-vs(n))**2 ) ) 
        vmag = vmag_old                              ! Wind speed (m s-1) 
        ...
        ! EQN (17):                                  
        !ssq = 0.98_R8 * qsat(ts(n),ps)               ! Surface specific humidity (kg kg-1)
        ssq = 0.98_R8 * qsat(ts(n),pslv(n))          ! Surface specific humidity (kg kg-1)
        ...
        visa = 1.326e-5_R8 * (1.0_R8 + &             ! Kinematic viscosity of dry
             & 6.542e-3_R8*tbot(n) + &               ! air (m2 s-1) from Andreas (1989)
             & 8.301e-6_R8*tbot(n)*tbot(n) - &       ! CRREL Rep. 89-11    
             & 4.84e-9_R8*tbot(n)*tbot(n)*tbot(n))  
     ...
     !-----Initial values of u* and convective velocity.-----------
        ...
        if (delthv.lt.0.0_R8) then
            ! EQN (19)
            vmag = sqrt( (ubot(n)-us(n))**2 + (vbot(n)-vs(n))**2 + &
                 & beta*beta*wstar*wstar )
        endif
     ...
     !-----Main iterations (2-10 iterations would be fine).-------
        do i=1,10
            ...
            call rough(zo,zot,zoq,ustar,visa)
        
            ! Wind variables.
            zeta = zbot(n) / obu
            if (zeta.lt.-zetam) then 
                ! EQN (7), very unstable regime
                ustar = loc_karman * vmag / (log(-zetam*obu/zo) - & 
                     & psi(1_IN, -zetam) + &
                     & psi(1_IN, zo/obu) + &
                     & 1.14_R8 * ((-zeta)**0.333_R8 -(zetam)**0.333_R8) )
            else if (zeta.lt.0.0_R8) then
                ! EQN (8), unstable regime
                ustar = loc_karman * vmag / (log(zbot(n)/zo) - psi(1_IN,zeta) + &
                        psi(1_IN,zo/obu))
            else if (zeta.le.1.0_R8) then 
                ! EQN (9), stable regime
                ustar = loc_karman * vmag / (log(zbot(n)/zo) + 5.0_R8*zeta - 5.0_R8*zo/obu)
            else 
                ! EQN (10), very stable regime
                ustar = loc_karman * vmag / (log(obu/zo) + 5.0_R8 - 5.0_R8*zo/obu + &
                     &             (5.0_R8*log(zeta) + zeta - 1.0_R8) )
            endif
  
            ! Temperature variables.
            if(zeta.lt.-zetat) then 
                ! EQN (11), very unstable regime
                tstar = loc_karman * delth / (log(-zetat*obu/zot) - &
                      & psi(2_IN, -zetat) + &
                      & psi(2_IN, zot/obu) + &
                      & 0.8_R8*((zetat)**(-0.333_R8) - (-zeta)**(-0.333_R8)) )
            else if (zeta.lt.0.0_R8) then 
                ! EQN (12), unstable regime
                tstar = loc_karman * delth / &
                      & (log(zbot(n)/zot) - psi(2_IN,zeta) + psi(2_IN,zot/obu))
            else if (zeta.le.1.0_R8) then 
                ! EQN (13), stable regime
                tstar = loc_karman * delth / (log(zbot(n)/zot) + 5.0_R8*zeta - 5.0_R8*zot/obu)
            else        
                ! EQN (14), very stable regime    
                tstar = loc_karman * delth / (log(obu/zot) + 5.0_R8 - 5.0_R8*zot/obu + &
                      & (5.0_R8*log(zeta) + zeta - 1.0_R8) )
            endif
            ...
            if (zeta.lt.-zetat) then 
                ! EQN (11), very unstable regime
                re = loc_karman / (log(-zetat*obu/zoq) - psi(2_IN,-zetat) + &
                      & psi(2_IN,zoq/obu) + &
                      & 0.8_R8*((zetat)**(-0.333_R8) - (-zeta)**(-0.333_R8)) )
            else if (zeta.lt.0.0_R8) then
                ! EQN (12), unstable regime  
                re = loc_karman / &
                      & (log(zbot(n)/zoq) - psi(2_IN,zeta) + psi(2_IN,zoq/obu))
            else if (zeta.le.1.0_R8) then 
                ! EQN (13), stable regime
                re = loc_karman / (log(zbot(n)/zoq) + 5.0_R8*zeta - 5.0_R8*zoq/obu)
            else  
                ! EQN (14), very stable regime
                re = loc_karman / (log(obu/zoq) + 5.0_R8 - 5.0_R8*zoq/obu + &
                      & (5.0_R8*log(zeta) + zeta - 1.0_R8) )
             endif
             qstar = re * delq
            ...
            if (delthv.lt.0.0_R8) then
                ! EQN (20)
                wstar = beta * (-loc_g*ustar*thvstar*zpbl/thv)**0.333_R8
                ! EQN (19)
                vmag = sqrt((ubot(n)-us(n))**2 + (vbot(n)-vs(n))**2 + wstar*wstar)
            endif
     ...
     !-----Calculate fluxes and wind stress.---------------------
        ...
        !--- momentum flux ---
        ! This should ensure zero wind stress when wind speed is zero, 
        ! components are consistent with total, and we don't ever 
        ! divide by zero. 
        ! However, it does have the side effect that for 0 < vmag_old < umin,
        ! we get tau < rbot(n) * ustar * ustar.
        ! EQN (21)
        tau = rbot(n) * ustar * ustar 
        taux(n) = tau * (ubot(n)-us(n)) / vmag_old
        tauy(n) = tau * (vbot(n)-vs(n)) / vmag_old
        ...
        zeta = zbot(n) / obu
        if (zeta.lt.0.0_R8) then
            ! Unstable case. 
            ! EQN (8)
            u10 = vmag + (ustar/loc_karman) * &
                & (log(zref/zbot(n)) - (psi(1_IN,zref/obu) - psi(1_IN,zeta)) )
            ! EQN (12)
            tref(n) = thbot(n) + (tstar/loc_karman) * &
                    & (log(ztref/zbot(n)) - (psi(2_IN,ztref/obu) - psi(2_IN,zeta)) )
            qref(n) = qbot(n) + (qstar/loc_karman) * &
                    & (log(ztref/zbot(n)) - (psi(2_IN,ztref/obu) - psi(2_IN,zeta)) )
        else
            ! Could also add very stable and very unstable cases?
            ! Stable case. 
            ! EQN (9)
            !u10 = vmag + (ustar/loc_karman) * &
            !    & (log(zref/zbot(n)) + 5.0_R8*zref/obu - 5.0_R8*zeta)
            ! Calculate from the surface up instead of zbot down.
            u10 = (ustar/loc_karman) * (log(zref/zo) + 5.0_R8*zref/obu )
            ! EQN (13)
            !tref(n) = thbot(n) + (tstar/loc_karman) * &
            ...
            !     & (log(ztref/zbot(n)) + 5.0_R8*ztref/obu - 5.0_R8*zeta)
            ! Calculate from the surface up instead of zbot down.
            tref(n) = ts(n) + (tstar/loc_karman) * &
                 (log(ztref/zot) + 5.0_R8*ztref/obu)
            qref(n) = ssq + (qstar/loc_karman) * &
                 (log(ztref/zoq) + 5.0_R8*ztref/obu)
        endif
...
!===============================================================================
   ...
   ! Buck(1981) JAM 20, 1527-1532
     ...
     real(R8) function qsat(t,p)
       ...
       qsat = loc_epsilon * esat / ((p/100.0_R8) - (1.0_R8 - loc_epsilon)*esat)
...
!===============================================================================
   !Calculate roughness lengths: zo, zot, zoq.
     ...
     subroutine rough(zo,zot,zoq,ustar,visa)
       ...
       xq = 2.67_R8*re**0.25_R8 - 2.57_R8                        ! EQN (25)
       xt = xq                                                   ! EQN (26)

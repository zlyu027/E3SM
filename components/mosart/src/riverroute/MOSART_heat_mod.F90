...
! !PUBLIC MEMBER FUNCTIONS:
	...
	use shr_sys_mod , only : shr_sys_flush
		...
		!if(TUnit%fdir(iunit) >= 0 .and. TUnit%areaTotal(iunit) > TINYVALUE1 .and. TUnit%tlen(iunit) >= TINYVALUE1) then
				...
				THeat%He_t(iunit) = cr_latentheat(THeat%forc_t(iunit), THeat%forc_pbot(iunit), THeat%forc_vp(iunit), THeat%forc_wind(iunit), 1._r8, THeat%Tt(iunit), TRunoff%tarea(iunit,nliq))
				THeat%Hh_t(iunit) = cr_sensibleheat(THeat%forc_t(iunit), THeat%forc_pbot(iunit), THeat%forc_wind(iunit), 1._r8, THeat%Tt(iunit), TRunoff%tarea(iunit,nliq))
	...
	! !DESCRIPTION: calculate the net heat balance of main channel.
		...
		!if(TUnit%fdir(iunit) >= 0 .and. TUnit%areaTotal(iunit) > TINYVALUE1 .and. TUnit%rlen(iunit) >= TINYVALUE1) then
				...
				THeat%He_r(iunit) = cr_latentheat(THeat%forc_t(iunit), THeat%forc_pbot(iunit), THeat%forc_vp(iunit), THeat%forc_wind(iunit), 1._r8, THeat%Tr(iunit), TRunoff%rarea(iunit,nliq))
				THeat%Hh_r(iunit) = cr_sensibleheat(THeat%forc_t(iunit), THeat%forc_pbot(iunit), THeat%forc_wind(iunit), 1._r8, THeat%Tr(iunit), TRunoff%rarea(iunit,nliq))
				...
				!end do
                THeat%Ha_rin(iunit) = THeat%Ha_rin(iunit) - THeat%HaroutUp(iunit)
			...
			if(iunit==12767) then
			    !write(unit=1112,fmt="(6(e14.4))") theDeltaT,THeat%Hs_r(iunit), THeat%Hl_r(iunit), THeat%He_r(iunit), THeat%Hc_r(iunit), THeat%Hh_r(iunit)
			    !write(unit=1113,fmt="(8(e14.4))") THeat%ha_lateral(iunit), THeat%Ha_rin(iunit), THeat%Ha_rout(12677), Ha_temp, TRunoff%erin(iunit,nliq),TRunoff%erin(iunit,nfrz),TRunoff%erlateral(iunit,nliq),TRunoff%erlateral(iunit,nfrz)
			    !write(unit=1113,fmt="(4(e14.4))") THeat%ha_lateral(iunit), cr_advectheat(TRunoff%erlateral(iunit,nliq)+TRunoff%erlateral(iunit,nfrz),THeat%Tr(iunit)), THeat%Ha_rin(iunit), cr_advectheat(TRunoff%erin(iunit,nliq)+TRunoff%erin(iunit,nfrz),THeat%Tr(iunit))
			    !write(unit=1115,fmt="(8(e14.4))") THeat%Tr(12677), -TRunoff%erout(12677,nliq), THeat%Tr(iunit), TRunoff%erin(iunit,nliq)
				!write(unit=1114,fmt="(8(e14.4))") THeat%forc_solar(iunit), TRunoff%rarea(iunit,nliq), THeat%forc_lwrad(iunit), THeat%Tr(iunit), THeat%forc_t(iunit), THeat%forc_pbot(iunit), THeat%forc_vp(iunit), THeat%forc_wind(iunit)
			end if
	...
	! !DESCRIPTION: calculate the water temperature of subnetwork channel.
		...
		real(r8) :: Ttmp1, Ttmp2  !
			...
			if((TRunoff%wt(iunit,nliq)+TRunoff%wt(iunit,nfrz)) > TINYVALUE1) then
				...
				if(TRunoff%qsur(iunit,nliq)+TRunoff%qsur(iunit,nfrz) > TINYVALUE1) then
	...
	! !DESCRIPTION: calculate the water temperature of subnetwork channel.
		...
		if(TRunoff%qsur(iunit,nliq)+TRunoff%qsub(iunit,nliq) > TINYVALUE1) then
			THeat%Tt(iunit) = THeat%Tqsur(iunit) * (TRunoff%qsur(iunit,nliq)+TRunoff%qsur(iunit,nfrz)) + THeat%Tqsub(iunit) * (TRunoff%qsub(iunit,nliq)+TRunoff%qsub(iunit,nfrz))
			THeat%Tt(iunit) = THeat%Tt(iunit)/(TRunoff%qsur(iunit,nliq)+TRunoff%qsur(iunit,nfrz)+TRunoff%qsub(iunit,nliq)+TRunoff%qsub(iunit,nfrz))
		else
			THeat%Tt(iunit) = THeat%Tqsur(iunit)
	...
	use shr_sys_mod , only : shr_sys_flush
		...
		real(r8) :: Ttmp1, Ttmp2  !
			...
			if((TRunoff%wr(iunit,nliq)+TRunoff%wr(iunit,nfrz)) > TINYVALUE1) then
		    ...
		    !if(THeat%Tr(iunit) < 200._r8 ) then  !TUnit%iUp(iunit,k)
			    !write(18,*)  iunit
			!end if
		    !if(iunit == 12767) then
			    !write(unit=1111,fmt="(2(I10), 2(f8.2),4(e18.4))") TUnit%iUp(iunit,1), TUnit%iUp(iunit,2), TRunoff%yr(iunit,nliq), THeat%Tr(iunit), THeat%Tr(TUnit%iUp(iunit,1)),THeat%Tr(TUnit%iUp(iunit,2)),THeat%deltaH_r(iunit), THeat%deltaM_r(iunit)
				!stop
			!end if

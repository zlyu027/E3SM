...
! !PUBLIC MEMBER FUNCTIONS:
	...
	use shr_sys_mod , only : shr_sys_flush
		...
		!if(TUnit%fdir(iunit) >= 0 .and. TUnit%areaTotal(iunit) > TINYVALUE1 .and. TUnit%tlen(iunit) >= TINYVALUE1) then
				...
				THeat%Hc_t(iunit) = cr_condheat(THeat%Hs_t(iunit),TRunoff%tarea(iunit,nt_nliq))
	...
	! !DESCRIPTION: calculate the net heat balance of main channel.
		...
		!if(TUnit%fdir(iunit) >= 0 .and. TUnit%areaTotal(iunit) > TINYVALUE1 .and. TUnit%rlen(iunit) >= TINYVALUE1) then
				if(TRunoff%yr(iunit,nt_nliq) > TUnit%rdepth(iunit)) then
				...
				THeat%Hc_r(iunit) = cr_condheat(THeat%Hs_r(iunit),TRunoff%rarea(iunit,nt_nliq))
				
				!do k=1,TUnit%nUp(iunit)
				!	THeat%Ha_rin(iunit) = THeat%Ha_rin(iunit) - THeat%Ha_rout(TUnit%iUp(iunit,k))
				!end do
				
				THeat%Ha_rin(iunit) = THeat%Ha_rin(iunit) - TRunoff%eroutUp(iunit,nt_rtm+1) 
			...
			if(iunit == 65846) then
			  write(unit=2111,fmt="(i10, 5(e20.11))") iunit, THeat%Hs_r(iunit), THeat%Hl_r(iunit), THeat%He_r(iunit), THeat%Hc_r(iunit), THeat%Hh_r(iunit)
			  write(unit=2112,fmt="(i10, 4(e20.11))") iunit, THeat%ha_lateral(iunit), THeat%Ha_rin(iunit), Ha_temp, TRunoff%erin(iunit,nt_nliq)
           end if
		!end if
	...
	! !DESCRIPTION: calculate the water temperature of subnetwork channel.
		...
		real(r8) :: Ttmp1, Ttmp2  !
			...
			if((TRunoff%wt(iunit,nt_nliq)+TRunoff%wt(iunit,nt_nice)) > TINYVALUE1) then
	...
	use shr_sys_mod , only : shr_sys_flush
		...
		real(r8) :: Ttmp1, Ttmp2  !
			...
			if((TRunoff%wr(iunit,nt_nliq)+TRunoff%wr(iunit,nt_nice)) > TINYVALUE1) then
			...
			!if(THeat%Tr(iunit) < 200._r8) then
			  ...
			  !write(unit=1112,fmt="(i10, 6(f10.4), 2(e20.11))") iunit, THeat%Tr(iunit), THeat%forc_t(iunit), THeat%Tqsur(iunit), THeat%Tqsub(iunit), THeat%forc_lwrad(iunit), THeat%forc_solar(iunit), THeat%deltaH_r(iunit), THeat%deltaM_r(iunit)

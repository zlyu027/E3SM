module lnd2atmMod
  ...
  use clm_varpar           , only : numrad, ndst, nlevsno, nlevgrnd, nlevsoi !ndst = number of dust bins.
...
contains
  ...
  subroutine lnd2atm(bounds, &
    ...
    type(soilhydrology_type), intent(in)     :: soilhydro_vars
    type(lnd2atm_type)     , intent(inout)  :: lnd2atm_vars 
    !
    ! !LOCAL VARIABLES:
    integer :: g, lvl             ! index
	...
	do lvl = -nlevsno+1, nlevgrnd
		call c2g( bounds, &
		    ...
		    c2l_scale_type= 'urbans', l2g_scale_type='unity' )
	enddo
    soilhydro_vars%zwt_col(:) = 1.0_r8
	lnd2atm_vars%zwt_grc  = 1.0_r8
    !call c2g( bounds, &
    !     soilhydro_vars%zwt_col (bounds%begc:bounds%endc), &
    ...
    do g = bounds%begg,bounds%endg
       ...
       if(lnd2atm_vars%t_soisno_grc(g, 1) > 1000._r8) then
	       do lvl = -nlevsno+1, nlevgrnd
	       lnd2atm_vars%t_soisno_grc(g, lvl) = 273.15_r8
		   end do
	   end if

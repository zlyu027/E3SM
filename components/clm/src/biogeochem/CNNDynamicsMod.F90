      ...
      do fc=1,num_soilc
          ...
          do p = col%pfti(c), col%pftf(c)
              if (pft%active(p).and. (pft%itype(p) .ne. noveg)) then
                  ...
                  r_fix = -6.25_r8*(exp(-3.62_r8 + 0.27_r8*t_soi10cm_col(c)*(1._r8-0.5_r8*t_soi10cm_col(c)/25.15_r8))-2._r8) 
                  ! calculate c cost of root n uptake: rastetter 2001, ecosystems, 4(4), 369-388.
                  r_nup = benefit_pgpp_pleafc(p) / max(pnup_pfrootc(p),1.e-20_r8)
                  ! calculate fraction of root that is nodulated: wang 2007 gbc doi:10.1029/2006gb002797
                  f_nodule = 1._r8 - min(1.0_r8,r_fix * r_nup/ (r_nup*r_nup+1.e-20_r8) )
                  ...
                  N2_aq = 0.78_r8 * 6.1e-4_r8 *28._r8 /1.e-3_r8 * h2osoi_vol(c,4)

...
!  real(r8):: clat(nelemd),latdeg(nelemd)
...
!     
   ...
   if (status .ne. nf90_noerr) then
      ...
      if ( have_tsair ) then
         ...
         tground = elem(1)%state%T(1,1,plev,3) 
      endif
   else
...
!               q3(1,i,inumliq,1,n3)=numliqobs(i)
                elem(1)%state%Q(1,1,i,inumliq)=numliqobs(i)
            end do
         endif
     endif
   end if
...
!            q3(1,i,icldliq,1,n3)=cldliqobs(i)
             elem(1)%state%Q(1,1,i,icldliq)=cldliqobs(i)
         end do
      endif
   endif
...
!            q3(1,i,icldice,1,n3)=cldiceobs(i)
             elem(1)%state%Q(1,1,i,icldice)=cldiceobs(i)
         end do
      endif
   endif
...
!               q3(1,i,inumice,1,n3)=numiceobs(i)
                elem(1)%state%Q(1,1,i,inumice)=numiceobs(i)
            end do
         endif
      endif
   end if

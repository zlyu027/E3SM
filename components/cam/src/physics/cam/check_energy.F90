...
!!! cam6    end do
    ...
    else if (ixrain > 1 .and. ixrim > 1) then  ! In P3 rim mass is part of ice mass, but rain is still needed.
       do k = 1, pver
          do i = 1, ncol
             wr(i) = wr(i) + state%q(i,k,ixrain)*state%pdel(i,k)/gravit
          end do
       ...
       if (any(abs(tw_rer(1:ncol)) > 1.E-10_r8)) then
          ...
          do i = 1, ncol
             if ( abs(tw_rer(i)) > 1.E-10_r8) then
                ...
                write(iulog,"(A20,1x,A15,1x,5(E15.7,1x))") 'Individual:', '', wv(i), wl(i), wr(i), wi(i), ws(i)  !Added by Aaron
             end if
          end do
       end if
    end if
